...
class DiffusionProcess():
    ...
    def sample(self, latents, kg_cond, img_cond, text_cond, class_labels=None):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            # Euler step.
            denoised = self.net(x_hat, kg_emb, img_emb, text_emb, t_hat, class_labels).to(torch.float64)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction.
            if i < self.num_steps - 1:
                denoised = self.net(x_next, kg_emb, img_emb, text_emb, t_next, class_labels).to(torch.float64)
    ...
    @torch.no_grad()
    def sampling(self, kg_cond, img_cond, text_cond, sampling_number=16, is_test=False, xT=None):
        if xT is None:
            xT = torch.randn([sampling_number, *self.shape]).to(device=self.device)

        if is_test:
            return self.sample(xT, kg_cond, img_cond, text_cond)
        else:
            return self.sample(xT, kg_cond, img_cond, text_cond)
