...
from KGDiff.testing_models.s4d import S4D, dropout_fn
...
def compute_classification_score(x_fake, x_real, get_optim_func, device):
    ...
    trainloader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(X_train, Y_train), shuffle=True,
                                              batch_size=1024)
    testloader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(X_test, Y_test), batch_size=1024)
...
def compute_predictive_score(x_real, x_fake, pred_step, get_optim_func, device, pred_activation):
    ...
    trainloader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(X, Y, masks), shuffle=True, batch_size=1024)
    testloader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(X_test, Y_test, masks), batch_size=1024)
    optimizer, _ = get_optim_func(model, lr=0.01, weight_decay=0.0, epochs=100)
